...
<meta charset="UTF-8">
<meta name="description" content="how to use json with php and ajax">
...
<?php
$our_data=array(
"mahdi"=>array("family"=>"khanzadi","phone"=>"0937xxxxxxx","address"=>"gorgan"),
"mohammad"=>array("family"=>"asgharian","phone"=>"0911xxxxxxx","address"=>"tehran"),
"sara"=>array("family"=>"hosseiny","phone"=>"0921xxxxxxx","address"=>"rasht")
);
if($_SERVER["REQUEST_METHOD"]=="POST"){
$post=!empty($_POST['data']) ? $_POST['data'] : null ;
$post=json_decode($post,JSON_UNESCAPED_UNICODE);
if(!empty($post) && is_array($post)){
if(!empty($our_data[$post['name']])){
echo json_encode($our_data[$post['name']],JSON_UNESCAPED_UNICODE);
}else{
...
p.appendChild(document.createTextNode(obj+"="+result[obj]));
result_elem.appendChild(p);
}
}catch(e){
...
var data={name:event.currentTarget.value};
console.log(data);
xhr.send("data="+JSON.stringify(data));
    ...
    <script>
	   $(document).ready(function () {
        $.getJSON("https://ed808.com:92/latin/intern?page=0&sort=last",
         function(data) {
             txt.textContent = data.contents[0].nid;
             for (let i = 0; i < data.count; i++) {
                 $('#dynamic').append(generateTable(data.contents[i]));
             }
          });
    });
        function generateTable(data) {
            return "<table><tr><td>" +
                   "<img src=" + data.picture + "></td></tr><tr><td align='center'>" +
                   data.title +
                   "</td></table>";
        }
